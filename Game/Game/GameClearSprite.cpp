...
GameClearSprite::GameClearSprite(const Vector2& pos, const Vector2& scale, const float& alpha)
...
{
	// テクスチャの生成
	mTexture = GraphicResourceManager::LoadTexture("Assets/UI/GameClear/GameClear.png");
...
void GameClearSprite::Update(float deltaTime)
{
	FadeIn(deltaTime);
}
...
void GameClearSprite::FadeIn(float deltaTime)
{
	if (IsAdition)
	{
