...
CurtainSprite::CurtainSprite(const Vector2& pos, const Vector2& scale, const float& alpha)
	:UIScreen(pos, scale, alpha)
	, mFadeInSpeed(0.0f)
	, mNormalFadeSpeed(0.1)
	, IsAdition(false)
{
	// テクスチャの生成
	mTexture = GraphicResourceManager::LoadTexture("Assets/UI/Curtain/Curtain.png");

	mFadeInSpeed = mNormalFadeSpeed;
}

CurtainSprite::~CurtainSprite()
{
}

void CurtainSprite::Update(float deltaTime)
{
}

void CurtainSprite::Draw()
{
	// 頂点配列をアクティブに変更
...
void CurtainSprite::FadeIn(float deltaTime)
{
	if (mAlpha >= 0.0f)
	{
...
void CurtainSprite::FadeOut(float deltaTime)
{
	if (mAlpha <= 1)
	{
		mAlpha += mFadeInSpeed * deltaTime;
